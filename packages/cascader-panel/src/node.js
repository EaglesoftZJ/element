import { isEqual, capitalize } from 'eg-element-ui-desk/src/utils/util';
...
export default class Node {
  ...
  initChildren() {
    ...
    this.hasChildren = Array.isArray(childrenData) && childrenData.length > 0; // 空数组也表示没有子节点
  ...
  get isDisabled() {
    ...
    return this.getIsDisabled(data[disabledKey] ||
  ...
  get isLeaf() {
    ...
    if (lazy) {
      const isLeaf = isDef(data[leafKey])
        ? data[leafKey]
        : (loaded ? this.getIsLeaf(!children.length) : false);
      this.hasChildren = !isLeaf;
      return isLeaf;
    }
    return this.getIsLeaf(!hasChildren);
  }
  // 增加isLeafFn判断 isLeafFn不存在则用传入的结果afterRes
  getIsLeaf(afterRes) {
    const { data } = this;
    const { isLeafFn } = this.config;
    return isLeafFn ? isLeafFn(data) : afterRes;
  }

  // 增加isLeafFn判断 isLeafFn不存在则用传入的结果afterRes
  getIsDisabled(afterRes) {
    const { data } = this;
    const { isDisableFn } = this.config;
    return isDisableFn ? isDisableFn(data) : afterRes;
  }
